...
public class ConverterSvcTest {
    ...
    public void getExchangeRate_USD_ReturnsUSDExchangeRate() throws IOException {
        ...
        //assert
        double expected = 11486.5341; //get the value from stream
        Assertions.assertEquals(expected, actual);
    }

    @Test
    public void getExchangeRate_GBP_ReturnsGBPExchangeRate() throws IOException {
        //arrange (our mocking setup)
        when(statusLine.getStatusCode()).thenReturn(200);
        when(response.getStatusLine()).thenReturn(statusLine);
        when(response.getEntity()).thenReturn(entity);
        when(entity.getContent()).thenReturn(stream);//whenever there is a call to get the content, then we are gonna return in our mock, the stream we previously set up
        when(client.execute(any(HttpGet.class))).thenReturn(response);
    ...
    @Test //when the api is offline. Expected error code is -1
